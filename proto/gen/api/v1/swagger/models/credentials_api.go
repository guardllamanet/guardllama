// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// CredentialsAPI credentials Api
//
// swagger:model CredentialsApi
type CredentialsAPI struct {

	// token
	Token string `json:"token,omitempty"`
}

// Validate validates this credentials Api
func (m *CredentialsAPI) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this credentials Api based on context it is used
func (m *CredentialsAPI) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *CredentialsAPI) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *CredentialsAPI) UnmarshalBinary(b []byte) error {
	var res CredentialsAPI
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
